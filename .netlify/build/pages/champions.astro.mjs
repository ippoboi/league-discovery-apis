...
const $$ChampionCard = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<a${addAttribute(`/champions/${data.name}`, "href")} class="relative overflow-hidden rounded-lg border border-zinc-900"> <p class="absolute bottom-1 left-1 text-white bg-zinc-900/80 border border-zinc-700 rounded-md px-2 py-0.5 z-10"> ${data.name} </p> ${renderComponent($$result, "Image", $$Image, { "src": imageUrl, "alt": data.name + " splash art", "height": 720, "width": 540, "class": "hover:scale-105 transition-all duration-300 grayscale-75 hover:grayscale-0", "loading": loadingStrategy })} </a>`;
